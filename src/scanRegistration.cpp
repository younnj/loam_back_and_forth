...
int64_t timeScanCur = 0;
int64_t timeScanLast = 0;
...
void laserCloudHandler(const std::shared_ptr<sensor_msgs::msg::PointCloud2> laserCloudIn2)
{
  std::cout << "[LOAM] Handling laser cloud" << std::endl;

  if (!systemInited) {
    initTime = laserCloudIn2->header.stamp.sec * 1e9 + laserCloudIn2->header.stamp.nanosec;
    imuPointerFront = (imuPointerLast + 1) % imuQueLength;
    systemInited = true;
  }

  timeScanLast = timeScanCur;
  timeScanCur = laserCloudIn2->header.stamp.sec*1e9 + laserCloudIn2->header.stamp.nanosec;
  ...
  for (int i = 0; i < cloudSize; i++) {
    ...
    laserCloud->points[i].h = static_cast<double>(timeLasted * 1e-9);
...
if (imuPointerLast >= 0) {
  while (imuPointerFront != imuPointerLast) {
    if (static_cast<double>(timeScanCur * 1e-9) < imuTime[imuPointerFront]) {  
      break;
    }
  ...
  if (static_cast<double>(timeScanCur * 1e-9) > imuTime[imuPointerFront]) {
    ...
    float ratioFront = (static_cast<double>(timeScanCur * 1e-9) - imuTime[imuPointerBack]) / (imuTime[imuPointerFront] - imuTime[imuPointerBack]);
    float ratioBack = (imuTime[imuPointerFront] - static_cast<double>(timeScanCur * 1e-9)) / (imuTime[imuPointerFront] - imuTime[imuPointerBack]);
...
imuShiftFromStartXCur = imuShiftXCur - imuShiftXStart - imuVeloXStart * (static_cast<double>(timeLasted * 1e-9) - static_cast<double>(timeStart * 1e-9));
imuShiftFromStartYCur = imuShiftYCur - imuShiftYStart - imuVeloYStart * (static_cast<double>(timeLasted * 1e-9) - static_cast<double>(timeStart * 1e-9));
imuShiftFromStartZCur = imuShiftZCur - imuShiftZStart - imuVeloZStart * (static_cast<double>(timeLasted * 1e-9) - static_cast<double>(timeStart * 1e-9));
...
void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuIn)
{
  ...
  //tf2::transformMsgToTF2(imuIn->orientation, orientation);
...
void sweepHandler(const std_msgs::msg::Empty::SharedPtr empty)   
{
  newSweep = true;   
}
