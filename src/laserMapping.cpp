...
void transformAssociateToMap()
{
  ...
  float crzcrx = sbcx * (cblx * sbly * (caly * salz - calz * salx * saly)
               ...
               + cbcx * cbcz * ((saly * salz + caly * calz * salx) * (sbly * sblz + cbly * cblz * sblx)
                                ...
                                + calx * calz * cblx * cblz) - cbcx * sbcz * ((saly * salz + caly * calz * salx)
...
int main(int argc, char** argv)
{
  ...
  auto subLaserCloudLast2 = nh->create_subscription<sensor_msgs::msg::PointCloud2>(          
    "/laser_cloud_last_2", rclcpp::QoS(2), laserCloudLastHandler);

  auto subLaserOdometry = nh->create_subscription<nav_msgs::msg::Odometry>(                  
    "/cam_to_init", rclcpp::QoS(5), laserOdometryHandler);

  const auto QOS = rclcpp::QoS(rclcpp::KeepAll()).reliable().durability_volatile();
  auto pubLaserCloudSurround = nh->create_publisher<sensor_msgs::msg::PointCloud2>(          
    "/laser_cloud_surround", QOS);

  auto pubOdomBefMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/bef_mapped_to_init_2", rclcpp::QoS(5));   
  auto odomBefMapped = std::make_shared<nav_msgs::msg::Odometry>();
  odomBefMapped->header.frame_id = "camera_init_2";
  odomBefMapped->child_frame_id = "bef_mapped";

  auto pubOdomAftMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/aft_mapped_to_init_2", rclcpp::QoS(5));   
  ...
  auto status = rclcpp::ok();
  while (status) {
    rclcpp::spin_some(nh);

    if (newLaserCloudLast && newLaserOdometry && std::fabs(timeLaserCloudLast - timeLaserOdometry) < 0.005) {
...
for (int i = 0; i < laserCloudFromMapNum; i++) {
  if (std::fabs(laserCloudFromMap->points[i].v - 2) < 0.005 ||
      std::fabs(laserCloudFromMap->points[i].v - 1) < 0.005) {
...
for (int i = 0; i < laserCloudLastNum; i++) {
  if (std::fabs(laserCloudLast->points[i].v - 2) < 0.005 ||
      std::fabs(laserCloudLast->points[i].v - 1) < 0.005) {
...
if (laserCloudSurfFromMap->points.size() > 500) {
  ...
  for (int iterCount = 0; iterCount < 10; iterCount++) {
    ...
    for (int i = 0; i < laserCloudLastNum; i++) {
      if (std::fabs(laserCloudLast->points[i].x > 0.3) || std::fabs(laserCloudLast->points[i].y > 0.3) ||
          std::fabs(laserCloudLast->points[i].z > 0.3)) {

        pointOri = laserCloudLast->points[i];
        pointAssociateToMap(pointOri, pointSel);
        if (std::fabs(pointOri.v) < 0.05 || std::fabs(pointOri.v + 1) < 0.05) {
          ...
          if (pointSearchSqDis[4] < 1.0) {
            ...
            if (planeValid) {
              ...
              float s = 1;
              if (iterCount >= 6) {
                s = 1 - 8 * std::fabs(pd2) / std::sqrt(std::sqrt(pointSel.x * pointSel.x
...
transformUpdate();
  ...
  laserCloudSurround2.header.stamp.sec = static_cast<int32_t>(floor(timeLaserCloudLast));
  laserCloudSurround2.header.stamp.nanosec = static_cast<int32_t>((timeLaserCloudLast-floor(timeLaserCloudLast)) * 1e9);
  ...
  geoQuat1.setRPY(transformBefMapped[2], -transformBefMapped[0], -transformBefMapped[1]);
  geometry_msgs::msg::Quaternion geoQuat = tf2::toMsg(geoQuat1);


  odomBefMapped->header.stamp.sec = static_cast<int32_t>(floor(timeLaserCloudLast));
  odomBefMapped->header.stamp.nanosec = static_cast<int32_t>((timeLaserCloudLast-floor(timeLaserCloudLast)) * 1e9);
  ...
  geoQuat1.setRPY(transformAftMapped[2], -transformAftMapped[0], -transformAftMapped[1]);
  geoQuat = tf2::toMsg(geoQuat1);

  odomAftMapped->header.stamp.sec = static_cast<int32_t>(floor(timeLaserCloudLast));
  odomAftMapped->header.stamp.nanosec = static_cast<int32_t>((timeLaserCloudLast-floor(timeLaserCloudLast)) * 1e9);
